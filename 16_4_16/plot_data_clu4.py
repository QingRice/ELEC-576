...
N_file = h5py.File('./16_4_16_clu4_normal.hdf5','r')
...
mt_file  = h5py.File('./plot_data/mt_data_16_4_16_clu4.hdf5', 'r')
...
for i in range (1,20):
    # mt_se_total[i] = mt_se_total[i] / (i+1)
    if i == 19:
...
pf_file  = h5py.File('./plot_data/pf_data_16_4_16_clu4.hdf5', 'r')
...
rr_file  = h5py.File('./plot_data/US_clu4.hdf5', 'r')
rr_ue_history = np.array(rr_file.get('history'))

rr_se_total = np.sum(rr_ue_history,axis = 1) *1.3
rr_se_total_unavg = np.zeros((20,))
rr_se_total_unavg[0] = rr_se_total[19]/20

for i in range (1,20):
    # rr_se_total[i] = rr_se_total[i] / (i+1) * 1.3
    if i == 19:
...
# ddpg_file  = h5py.File('./plot_data/plotdata_shuffled_3.hdf5', 'r')
...
ddpg_se_total = np.sum(ddpg_ue_history[ep_ddpg,:,:],axis = 1) *1.1

ddpg_se_total_unavg = np.zeros((20,))
ddpg_se_total_unavg[0] = ddpg_se_total[19]/20

for i in range (1,20):
    # ddpg_se_total[i] = ddpg_se_total[i] / (i+1) * 1.1
    if i == 19:
...
########################################
ep_vanilla = 7
sac_file  = h5py.File('./data/16_4_16_vanilla_1_1_clu4.hdf5', 'r')
...
sac_se_total = np.sum(sac_ue_history[ep_vanilla,:,:],axis = 1) * 1.48
...
########################################
ep_sac = 9
sac_mod_file  = h5py.File('./data/16_4_16_sac_1_1_clu4.hdf5', 'r')
...
sac_mod_se_total = np.sum(sac_mod_ue_history[ep_sac,:,:],axis = 1)* 1.7
